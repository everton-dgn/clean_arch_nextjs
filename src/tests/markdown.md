...
Diretório para **testes automatizados e2e**, cobrindo fluxos e páginas.

## Subdiretórios

- **`flows`**: Testes de integração para fluxos completos de usuário.
- **`helpFunctions`**: Funções auxiliares para testes.
- **`mocks`**: Dados ou estilos simulados.
- **`pages`**: Testes para páginas específicas.
- **`providers`**: Provedores para componentes e hooks.
